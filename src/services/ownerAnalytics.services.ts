...
export async function getAnalyticsSummaryService({ ownerId }: { ownerId: number }) {
    try {
        ...
        const currentMonthRevenue = currentMonthBookings.reduce((sum, booking) => sum + booking.totalPayableAmount, 0);
        ...
        const previousMonthRevenue = previousMonthBookings.reduce((sum, booking) => sum + booking.totalPayableAmount, 0);
    ...
    try {
        ...
        const villaPerformances = await Promise.all(
            villas.map(async (villa) => {
                ...
                const monthlyRevenue = bookings.reduce((sum, booking) => sum + booking.totalPayableAmount, 0);
...
export async function getMonthlyRevenueService({ ownerId }: { ownerId: number }) {
    try {
        ...
        for (let i = 0; i < monthsToFetch; i++) {
            ...
            const revenue = bookings.reduce((sum, booking) => sum + booking.totalPayableAmount, 0);
